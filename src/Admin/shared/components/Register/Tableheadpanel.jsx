...
export default function Tableheadpanel(props) {
  ...
  const [filterOptions, setFilterOptions] = useState({}); // Manage options for each field separately

  const getOption = async (key) => {
    const response = await getFilterOptions(key.field);
    const formattedOptions = response[key.field]?.map((val) => ({
      label: val,
      value: key.format === "Date" ? moment(val).format("YYYY-MM-DD") : val,
    })) || [];

    // Update options for the specific field
    setFilterOptions((prev) => ({
      ...prev,
      [key.field]: formattedOptions,
    }));
  };
    ...
    return (
      <div onClick={() => getOption(key)}>
        <MultiSelect
          ...
          options={filterOptions[fieldKey] || []} // Use options specific to the field
          optionLabel="label"
          ...
          placeholder={`Select ${fieldKey.charAt(0).toUpperCase() + fieldKey.slice(1)}`}
          panelFooterTemplate={
  ...
  return (
    <div className="flex flex-wrap gap-2 items-center justify-between px-6 py-4">
      <div>
        <h2 className="text-xl font-semibold text-gray-800 mb-2">Registrations</h2>
      </div>
      ...
      <div className="flex flex-wrap gap-2 items-center gap-x-2 *:border *:rounded-lg">
        ...
        <div
          ...
          className="group flex justify-center items-center gap-1 border rounded-lg p-2 px-5 cursor-pointer bg-green-700 text-white hover:text-green-700 hover:bg-white duration-150"
        >
          <div className="hidden lg:block">Export</div>
          <i className="fa-solid fa-download group-hover:animate-bounce text-xl"></i>
        </div>
      </div>
    </div>
  );
