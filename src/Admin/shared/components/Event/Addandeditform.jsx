    ...
    <Dialog header={!formdata?.id ? "Add Event" : "Update Event"} visible={visible} onHide={() => setVisible(false)} className="!w-full lg:!w-[40rem]" >
      <form onSubmit={!formdata?.id ? handlesave : handleupdate}>
            ...
            <label className=" relative z-[1] flex flex-col items-center justify-center w-40 h-40 sm:w-60 sm:h-60 overflow-hidden rounded-xl border-2 border-gray-300 border-dashed cursor-pointer bg-gray-50 dark:hover:bg-bray-800 dark:bg-gray-700 hover:bg-gray-100 dark:border-gray-600 dark:hover:border-gray-500 dark:hover:bg-gray-600">
              {dataUrl ? (
                <div className="mb-5">
                  ...
                  <i onClick={() => { setDataUrl(null); setFormdata((prev) => ({ ...prev, Image: null })); }}
                    className="fa-solid fa-circle-xmark absolute top-3 right-3 text-red-600 text-2xl z-20" >
                  </i>
                </div>
              ) : formdata && formdata?.Image ? (
                <div className="mb-5">
                  <img src={`${apiurl()}/${formdata.Image}`} alt="" />
                  <i onClick={() => { setFormdata((prev) => ({ ...prev, Image: null })); }}
                    className="fa-solid fa-circle-xmark absolute top-3 right-3 text-red-600 text-2xl z-20" >
                  </i>
                </div>
              ) : (
            ...
            <>
                  ...
                  <div className="lg:col-span-2" key={index}>
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
                      ...
                      <div className="text-end">
                        <button type="button" onClick={(e) => removeGame(e, index)} className=" px-2 py-1 text-sm text-white bg-danger-600 border rounded-md text-center" >
                          {" "}
              ...
              <div className="lg:col-span-2 text-end">
                <button type="button" onClick={AddGame} className="px-4 py-2 text-white bg-secondary border rounded-md text-center" >
                  ...
                  <span className="hidden md:block">+ Add Subevent</span>
                </button>
              </div>
        ...
        <div className="mb-2">
          <button
            type="submit"
            className="w-full px-4 py-2 text-white bg-secondary border rounded-md"
          >
            {loading && (
